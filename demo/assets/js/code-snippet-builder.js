class CodeSnippet {
    ...
    highlightHTML(htmlString) {
        let insideTag = false;
        return htmlString
          .replace(/(&lt;!--[\s\S]*?--&gt;|&lt;style&gt;[\s\S]*?&lt;\/style&gt;|&lt;\/?[\w-]+|&gt;|[\w-]+="[^"]*"|[\w-]+|[^&<>]+)/g, match => {
              if (match.startsWith('&lt;!--') && match.endsWith('--&gt;')) {
                  ...
                  return match
                    .replace(/(&lt;style&gt;)([\s\S]*?)(?=&lt;\/style&gt;)/g, (full, openTag, content) =>
                      `<span class="symbol">&lt;</span><span class="tag">style</span><span class="symbol">&gt;</span><span class="style-content">${content}</span>`)
                    .replace(/&lt;\/style&gt;/g, '<span class="symbol">&lt;</span><span class="tag">/style</span><span class="symbol">&gt;</span>');
