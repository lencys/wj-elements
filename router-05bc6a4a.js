...
let router$1;
const delegate$1 = function(el, eventName, selector, listener, context) {
...
const undelegate$1 = function(el, handler) {
...
function updateActiveClass(el, routeName, params, query) {
  ...
  if (activeClass) {
    const isActive = router$1.isActive(routeName, params, query, el.hasAttribute("exact"));
...
function updateLink(el, ownerEl) {
  ...
  try {
    const href = router$1.generate(routeName, params, query);
    const anchorEl = el.tagName === "A" ? el : el.querySelector("a");
    if (anchorEl)
      anchorEl.setAttribute("href", href);
    if (!router$1.state.activeTransition) {
...
function linkClickHandler(e) {
  ...
  router$1[method](routeName, params, query);
}
function bindRouterLinks(ownerEl, options = {}) {
  ...
  rootEls.forEach((rootEl) => {
    eventHandlers.push(delegate$1(rootEl, "click", "[route]", linkClickHandler, ownerEl));
  ...
  return function() {
    ...
    eventHandlers.forEach((eventHandler, i) => undelegate$1(rootEls[i], eventHandler));
  };
}
const createClass = (ctor, options = {}) => {
  return class extends ctor {
    connectedCallback() {
      ...
      const renderWait = this.updateComplete || resolved$1;
      renderWait.then(() => {
