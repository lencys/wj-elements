...
class v extends E {
  ...
  draw(e, t, r) {
    ...
    o.setAttribute('type', 'text'),
      o.setAttribute('part', 'input'),
      o.setAttribute('autocomplete', 'off'),
      o.setAttribute('readonly', ''),
      o.setAttribute('placeholder', this.placeholder || '');
    ...
    let j = document.createElement('slot'),
      h = document.createElement('wje-button');
    ...
    return (
      l.setAttribute('placement', 'bottom-start'),
      l.setAttribute('manual', ''),
      l.setAttribute('size', ''),
      this.hasAttribute('disabled') && l.setAttribute('disabled', ''),
      this.variant === 'standard' ? this.hasAttribute('label') && s.appendChild(m) : p.appendChild(m),
      d.appendChild(o),
      this.hasAttribute('multiple') && d.appendChild(b),
      this.hasAttribute('clearable') && d.appendChild(h),
      d.appendChild(w),
      c.appendChild(j),
      p.appendChild(d),
      l.appendChild(p),
      l.appendChild(c),
      this.trigger === 'click' && l.setAttribute('manual', ''),
      s.appendChild(l),
      (this.native = s),
      (this.popup = l),
      (this.labelElement = m),
      (this.input = o),
      (this.optionsWrapper = c),
      (this.chips = b),
      (this.clear = h),
      i.appendChild(s),
      i
  ...
  afterDraw(e, t, r) {
    this.input.addEventListener('focus', (i) => {
      ...
      this.clear.addEventListener('wje-button:click', (i) => {
      ...
      this.selections(),
      this.optionsWrapper.addEventListener('wje:options-load', (i) => {
  ...
  getSelected(e) {
    ...
    return (
      (t = Array.isArray(t) ? t : Array.from(t)),
      (t = t.map((r) => ({
        value: r.value,
  ...
  selectionChanged(e = null, t = 0) {
    if (this.hasAttribute('multiple'))
      (this.value = this.selectedOptions.map((r) => r).reverse()),
        this.placeholder && t === 0
          ? ((this.chips.innerHTML = this.placeholder), (this.input.value = ''))
          : this.counterEl instanceof HTMLElement || t > +this.maxOptions
            ? this.counter()
            : e != null && this.chips.appendChild(this.getChip(e));
    else {
  ...
  selections(e) {
    let t = this.getSelectedOptions();
    (this.selectedOptions = Array.isArray(t) ? t : Array.from(t)),
      this.selectedOptions.length >= +this.maxOptions && (this.counterEl = null),
      (this.chips.innerHTML = ''),
      this.selectedOptions.length > 0
        ? this.selectedOptions.forEach((r, i) => {
