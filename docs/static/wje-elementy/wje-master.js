...
const A = ".container{position:relative;display:flex;align-items:center;justify-content:center;flex-direction:var(--flex-direction)}.container-w-dropzones{position:relative;display:flex;align-items:center;justify-content:center;flex-direction:row}.reversed{--flex-direction: column-reverse}.basic{--flex-direction: column}";
class p extends f {
  constructor() {
...
class u extends f {
  ...
  draw(t, o, n) {
    ...
    if (s.classList.add("name"), v.hasSlot(this, "handle")) {
      ...
      i.classList.add("handle"), r.classList.add("item-w-handle"), i.setAttribute("name", "handle"), i.setAttribute("part", "handle-part"), r.appendChild(i);
    } else
