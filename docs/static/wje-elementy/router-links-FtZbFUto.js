const h = Symbol('routerLinksData'),
  l = Symbol('unbindRouterLinks'),
  p = Promise.resolve(),
  f = /* @__PURE__ */ new Set();
let u;
const y = function (t, r, e, n, s) {
  const a = function (c) {
    for (var o = c.target; o; o = o.parentNode) o.matches && o.matches(e) && ((c.selectorTarget = o), n.call(s, c));
  };
...
function i(t, r, e, n, s) {
  const a = t[h].options,
    c = N(t, r, n, e, a),
    o = e.closest(a.selector || '[routerlinks]');
...
function d(t, r) {
  ...
  if (!e) return;
  const n = i(r, e, t, 'params', 'param-'),
    s = i(r, e, t, 'query', 'query-');
  try {
    const a = u.generate(e, n, s),
      c = t.tagName === 'A' ? t : t.querySelector('a');
...
function E(t) {
  if (t.button !== 0 || v(t)) return;
  t.preventDefault();
  const r = t.selectorTarget,
    e = r.getAttribute('route');
  if (!e) return;
  const n = i(this, e, r, 'params', 'param-'),
    s = i(this, e, r, 'query', 'query-'),
    a = r.hasAttribute('replace') ? 'replaceWith' : 'transitionTo';
  u[a](e, n, s);
}
function S(t, r = {}) {
  const { selector: e = '[routerlinks]' } = r,
    n = e ? t.querySelectorAll(e) : [t],
    s = new MutationObserver(L),
    a = [];
  return (
    (s.ownerEl = t),
    (t[h] = { options: r, rootEls: n, observer: s }),
    n.forEach((c) => {
...
const m = (t, r = {}) =>
  ...
  x = (t, r) => {
    if (typeof t == 'function') return m(t, r);
    if (t.kind === 'class') {
...
function T() {
  f.forEach((t) => {
    t[h].rootEls.forEach((e) => {
      ...
      if (!n) return;
      const s = i(t, n, e, 'params', 'param-'),
        a = i(t, n, e, 'query', 'query-');
      g(e, n, s, a);
    });
  });
}
