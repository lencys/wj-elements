...
function u(s, e) {
  function t(n) {
    const r = s.getBoundingClientRect(),
      ...
      d = r.left + i.pageXOffset,
      v = r.top + i.pageYOffset,
      w = n.pageX - d,
      a = n.pageY - v;
...
class o extends m {
  ...
  constructor() {
    ...
    c(this, 'handleDrag', (t) => {
      ...
      u(this, {
        onMove: (l, n) => {
          let r = this.hasAttribute('vertical') ? n : l,
            i = this.pixelsToPercentage(r),
            d = 100 - this.pixelsToPercentage(r);
          this.style.setProperty('--wje-split-view-calc-a', i + '%'),
            this.style.setProperty('--wje-split-view-calc-b', d + '%');
        },
        initialEvent: t,
      });
    });
  ...
  draw(t, l, n) {
    ...
    return (
      a.classList.add('wje-divider'),
      a.setAttribute('part', 'divider'),
      a.appendChild(w),
      a.addEventListener('mousedown', this.handleDrag, !1),
      i.appendChild(d),
      i.appendChild(a),
      i.appendChild(v),
      r.appendChild(i),
  ...
  afterDraw() {
    this.detectSize(),
      this.min && this.style.setProperty('--wje-split-view-min', this.pixelsToPercentage(this.min) + '%'),
      this.max && this.style.setProperty('--wje-split-view-max', 100 - this.pixelsToPercentage(this.max) + '%'),
      this.initial &&
