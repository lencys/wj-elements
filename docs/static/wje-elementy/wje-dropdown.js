...
class p extends h {
  ...
  draw(e, g, w) {
    ...
    let c = document.createElement('slot'),
      i = document.createElement('wje-popup');
    return (
      i.setAttribute('placement', this.placement),
      i.setAttribute('offset', this.offset),
      i.appendChild(n),
      i.appendChild(c),
      this.trigger === 'click' && i.setAttribute('manual', ''),
      o.appendChild(i),
      l.appendChild(o),
      (this.popup = i),
      (this.anchorSlot = n),
  ...
  afterDraw() {
    this.trigger != 'click' &&
      (a.addListener(this.anchorSlot, 'mouseenter', null, this.onShow),
      a.addListener(this.anchorSlot, 'mouseleave', null, this.onHide)),
      this.hasAttribute('collapsible') &&
        a.addListener(Array.from(this.querySelectorAll('wje-menu-item')), 'click', 'wje-menu-item:click', this.onHide);
  }
}
