...
class u extends b {
  ...
  constructor() {
    ...
    s(this, 'setTextareaHeight', () => {
      this.getAttribute('resize') === 'auto' &&
        ((this.input.style.height = 'auto'), (this.input.style.height = this.input.scrollHeight + 'px'));
    });
    /**
  ...
  draw(t, w, m) {
    ...
    r.classList.add('native-textarea', this.variant || 'default'),
    ...
    if (
      ...
      e.setAttribute('part', 'input'),
      e.setAttribute('rows', this.rows || 3),
      e.setAttribute('spellcheck', !1),
      this.resize === 'auto' && e.addEventListener('input', this.setTextareaHeight),
      this.variant === 'standard' ? this.label && r.appendChild(o) : n.appendChild(o),
      n.appendChild(e),
      c.appendChild(n),
      r.appendChild(c),
      l.appendChild(r),
      this.hasAttribute('counter'))
    ) {
      (e.maxLength = this.maxLength || 1e3), e.addEventListener('input', this.counter);
      let d = document.createElement('div');
      d.classList.add('counter'),
        (d.innerText = `${e.value.length}/${e.maxLength}`),
        (this.counterElement = d),
        l.appendChild(d);
    }
  ...
  afterDraw() {
    (this.resizeObserver = new ResizeObserver(() => this.setTextareaHeight)),
      this.hasAttribute('disabled') ||
        (h.addListener(this, 'click', 'wje:textarea:change'), h.addListener(this, 'click', 'wje:textarea:input')),
