...
class f extends m {
  ...
  draw(e, i, n) {
    ...
    return (
      ...
      this.navigation &&
        ...
        t.appendChild(this.prevButton),
        t.appendChild(this.nextButton)),
      this.pagination && t.appendChild(this.createPagination()),
      this.thumbnails && t.appendChild(this.createThumbnails()),
      r.appendChild(t),
      (this.slides = s),
      r
    );
  ...
  afterDraw() {
    this.setIntersectionObserver(),
      this.getSlidesWithClones().forEach((i, n) => {
        this.intersectionObserver.observe(i);
      }),
      (this.slidePerPage = this.getAttribute('slide-per-page') || 1);
    let e = 100 / +this.slidePerPage;
    this.style.setProperty('--wje-carousel-size', e + '%'),
      this.goToSlide(this.activeSlide, 'auto'),
      this.slides.addEventListener('scrollend', (i) => {
        const n = this.getSlides(),
          t = [...this.entriesMap.values()].find((s) => s.isIntersecting);
  ...
  goToSlide(e, i = 'smooth', n = !0) {
    const r = this.getSlides(),
      t = this.getSlidesWithClones();
    t.forEach((g, S) => {
      g.classList.remove('active');
    });
    let s = this.loop ? (e + r.length) % r.length : Math.min(Math.max(e, 0), r.length - 1);
    this.activeSlide = s;
    const l = Math.min(Math.max(e + (this.loop ? this.slidePerPage : 0), 0), t.length - 1),
      c = this.getSlidesWithClones()[l];
    c.classList.add('active');
    let d = c.getBoundingClientRect(),
      h = this.slides.getBoundingClientRect();
    this.slides.scrollTo({
      ...
      this.navigation &&
        ...
        this.activeSlide === r.length - 1 && this.nextButton.setAttribute('disabled', ''),
        this.activeSlide === 0 && this.prevButton.setAttribute('disabled', ''));
  }
  ...
  removeActiveSlide() {
    this.getSlidesWithClones().forEach((e, i) => {
      ...
      this.pagination &&
        this.context.querySelectorAll('.pagination-item').forEach((e) => {
          e.classList.remove('active');
        }),
      this.thumbnails &&
        this.context.querySelectorAll('wje-thumbnail').forEach((e) => {
          e.classList.remove('active');
        });
  }
  changePagination() {
    this.pagination &&
      ...
      this.context.querySelectorAll('.pagination-item').forEach((e, i) => {
  ...
  changeThumbnails() {
    this.thumbnails &&
      (this.removeActiveSlide(),
      this.context.querySelectorAll('wje-thumbnail').forEach((e, i) => {
        i === this.activeSlide && e.classList.add('active');
      }));
  }
  createNextButton() {
    ...
    return (
      ...
      (e.innerHTML = '<wje-icon name="chevron-right" size="large"></wje-icon>'),
      e.setAttribute('circle', ''),
      e.setAttribute('fill', 'link'),
  ...
  createPreviousButton() {
    ...
    return (
      ...
      (e.innerHTML = '<wje-icon name="chevron-left" size="large"></wje-icon>'),
      e.setAttribute('circle', ''),
      e.setAttribute('fill', 'link'),
      e.addEventListener('click', (i) => {
        this.previousSlide();
  ...
  createPagination() {
    ...
    return (
      ...
      this.getSlides().forEach((n, r) => {
        const t = document.createElement('div');
        t.classList.add('pagination-item'),
  ...
  createThumbnails() {
    const e = document.createElement('div');
    return (
      e.classList.add('thumbnails'),
      this.getSlides().forEach((n, r) => {
        const t = document.createElement('wje-thumbnail');
        (t.innerHTML = `<img src="${n.querySelector('wje-img').getAttribute('src')}"></img>`),
          t.addEventListener('click', (s) => {
