...
class u extends f {
  ...
  constructor(t = {}) {
    ...
    d(this, 'onScroll', (t) => {
      ...
      i + n >= e - 300 &&
        ...
        (this.currentPage++, this.loadPages(this.currentPage));
    });
    (this.totalPages = 0),
      (this.isLoading = []),
      (this._response = {}),
      (String.prototype.interpolate = function (i) {
        let e = this,
          n = e.match(/\{{.*?\}}/g);
        if (n)
          for (let s of n) {
            let c = s.replace('{{', '').replace('}}', ''),
              a = '';
            console.log('CLEAN KEY', c),
              c.split('.').forEach((o) => {
  ...
  async getPages(t) {
    ...
    const e = await fetch(
      `${this.url}${i ? '&' : '?'}page=${t}&size=${this.size}${this == null ? void 0 : this.queryParams}`
    );
    if (!e.ok) throw new Error(`An error occurred: ${e.status}`);
    return await e.json();
  }
  /**
   * Hides the loader.
  ...
  async loadPages(t) {
    ...
    try {
      if (this.hasMorePages(t) || typeof this.setCustomData == 'function') {
        let i, e;
        typeof this.setCustomData == 'function' ? (e = await this.setCustomData(t)) : (e = await this.getPages(t)),
          (this.totalPages = e.totalPages),
          (this.currentPage = t);
        const n = new DOMParser();
        let s = this;
        this.hasAttribute('placement') && (s = this.querySelector(this.placement)),
          g.dispatchCustomEvent(this, 'wje-infinite-scroll:load', e),
          (this.response = e),
          e[this.objectName].forEach((c) => {
            const a = this.infiniteScrollTemplate.interpolate(c),
              h = n.parseFromString(a, 'text/html').querySelector(this.iterate.tagName.toLowerCase());
            g.addListener(h, 'click', 'wje-infinite-scroll:click-item', null, { stopPropagation: !0 }),
              s.insertAdjacentElement('beforeend', h);
          }),
          this.isLoading.push(t);
      } else g.dispatchCustomEvent(this, 'wje-infinite-scroll:complete'), this.endingEl.classList.add('show');
    } catch (i) {
