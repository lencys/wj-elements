...
class c extends v {
  ...
  draw(e, i, t) {
    ...
    if (
      (s.setAttribute('part', 'native'), s.classList.add('native-rate'), (this.native = s), this.hasAttribute('icons'))
    ) {
      let h = this.icons;
      for (let l = 0; l < h.length; l++) s.appendChild(this.createIcons(l));
    } else for (let h = 0; h < this.max; h++) s.appendChild(this.createIcons(h));
   ...
   */
  changeRate() {
    const e = this.native.children,
      i =
        this.value !== this.hoverValue && this.hoverValue !== 0 && this.hoverValue !== void 0
          ? this.hoverValue
          : this.value;
    for (let t = 0; t < e.length; t++)
      if (
        (e[t].classList.remove('selected', 'hovered'),
        e[t].children.length > 1 &&
          (e[t].classList.remove('half'),
          e[t].querySelector('wje-icon:first-child').removeAttribute('style'),
          e[t].querySelector('wje-icon:last-child').remove()),
        t < i && e[t].classList.add('selected'),
        i > t && i < t + 1 && e[t].children.length === 1)
      ) {
        let a = e[t].querySelector('wje-icon').cloneNode(!0);
        e[t].appendChild(a);
        let s = (i - t) * 100;
        e[t].classList.add('half'),
          (e[t].querySelector('wje-icon:first-child').style.clipPath = `inset(0 0 0 ${s}%)`),
          (e[t].querySelector('wje-icon:last-child').style.clipPath = `inset(0 ${s}% 0 0)`);
      }
  }
  /**
  ...
  getValueFromXPosition(e) {
    const { left: i, right: t, width: a } = this.native.getBoundingClientRect(),
      s = this.roundToPrecision(((e - i) / a) * this.max, this.precision);
